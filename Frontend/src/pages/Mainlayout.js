...
const MainLayout = ({ role }) => {
  ...
  return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div
            className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}
        >
          <div className="p-4 flex flex-col h-full">
            ...
            <div className="flex-grow">
              <p className="text-gray-700">Logged in as: {role.charAt(0).toUpperCase() + role.slice(1)}</p>
              {role !== "resident" && (
                  <div className="mt-4">
                    <p className="text-gray-700">You have access to Notifications.</p>
                  </div>
              )}
            </div>
            <button onClick={handleLogout} className="flex items-center space-x-2 w-full p-3 hover:bg-gray-100 rounded-lg transition-colors">
